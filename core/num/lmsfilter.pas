...
FUNCTION FastConv(x,h:TVectors):TVectorsBase;
{This function makes a very fast convolution of two vectors. The first "x" is
...
FUNCTION Wiener(VAR h:TVectors;x,d:TVectors;mu:Real):TVectors;
...
PROCEDURE LMS(VAR y,e:Real;VAR h:TVectors;x:TVectors;d,mu:Real);
IMPLEMENTATION
FUNCTION FastConv(x,h:TVectors):TVectorsBase;
  VAR
    i:Word;
...
PROCEDURE LMS(VAR y,e:Real;VAR h:TVectors;x:TVectors;d,mu:Real);
  BEGIN
    ...
    Add(h,2*mu*e,x);{"h:=h+2*mu*e*x" : Then we applay LMS algorithme to correct 
		   ...
		   details see <A HREF="realvectors.pas.html">realVectors</A>.}
  END;
FUNCTION Wiener(VAR h:TVectors;x,d:TVectors;mu:Real):TVectors;
  VAR
    i:Word;
